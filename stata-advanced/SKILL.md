...
compatibility: >-
  ...
  触发即读本文，无需联网加载其他文件。factor / sem / mi / mixed / irt 等 Stata 内置；Alpha 信度内置。
---
...
- [ ] 因子分析 PCF 与 PF 都跑了；先 KMO/碎石图再定因子数；未删条目提 α（删条目必须给理论理由）
- [ ] 信度报告：IRT 用 `irtgraph tif` 报 θ 区段信度，不报单一 α；评分者一致性用 `kap` 不是 `kappa`
- [ ] `sem`/`gsem` 路径系数没有用「导致/因果」措辞（仅「相关/关联」）；因果另有识别策略
- [ ] 多重插补：交互/平方项在 `mi set` 之前生成；`mi impute chained` 带辅助变量支撑 MAR；插补值未事后修正
- [ ] 多层模型：组数 ≥ 20–30（不足则转 panel 或 `cluster(group)` 稳健 SE）；时间变量大数已中心化
- [ ] log 恰好一次 `end of do-file`，无 `r(错误码)`（r(430)/r(420)/r(1400) 已按错误码速查排掉）
