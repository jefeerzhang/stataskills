...
- Agent 行为回归：`test-prompts.json` 38 条 prompt 三层模式——docs（CI 静态断言）/ `--prompts`（Stata 子集，需本机 Stata）/ `--llm`（真实 Agent，需 claude CLI 且 API key 或 OAuth 登录态任一）。`--llm` 台账中的 25/27 结果对应 2026-08-27 当时的 prompt corpus；后续动态面板、DCE、结果模型与敏感性分析 prompt 尚未重跑 LLM 层；台账 `verify/llm-results.md`、`verify/llm-smoke-results.md`。
