...
- `sort` 按估计值排序；`sort(, descending)` 降序；`sort(, by(se))` 按标准误排序；`sort(1, descending)` 指定系列排序。
...
- 括号内：`(m1, opts1 \ m2, opts2)` 中 `opts2` 作用于两个模型；只作用于 `m2` 写 `(m1, opts1 \ m2, opts2 \)`。
