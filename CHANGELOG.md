...
- docs(identification) 未观测混杂敏感性分析：新增 `stata-identification/references/sensitivity-analysis.md`（三类问题分流 → `psacalc` / `sensemakr` / `konfound` / `regsensitivity` / `evalue` / `rbounds` 命令总表 → 语法 → 必报量 → 8 条陷阱四件套 → 踢走规则）；新增 `verify/verify-sensitivity.do`（3 个内置闭式恒等式 OVB / partial R² / E-value 进 `VERIFY_MARKERS_REQUIRED`；6 个可选社区包交叉校验：`evalue` 复现 E-value 闭式、`sensemakr` 的 `e(r2yd_x)` 等于内置 partial R²、`psacalc delta`(rmax=1) 与 `regsensitivity bounds, oster` 给出同一 Oster δ=1.08701819）；`targets.sh` 新增第三个非恒等 plan（`verify-identification` 委托 `verify-sensitivity`）+ `community.sh` 登记 6 个 optional 包；`SKILL.md` references 3→4 + 触发词；`test-prompts.json` +1 条 prompt（prompt corpus 38 条）；ADR-0004 委托表与 README 计数同步。本机 StataNow 19.5 MP 实测并回写文档：`rmax()` 只接受 `(R²_controlled, 1]` 的字面量（`rmax(1.3)`、表达式、标量名均报错），须写 `` rmax(`=min(1.3*e(r2),1)') ``；`sensemakr` 强 benchmark 触发 `r(3498)`；`regsensitivity` 必须有 `depvar indepvar controls`；`konfound` 的 `r(rir)`、`rbounds` 的 `r(N)`/`r(outmat)` 已断言。
- 新增 `stata-count`、`stata-fractional`、`stata-limited-dependent`：计数/PPML、分数响应、删失/hurdle/样本选择估计与诊断。各自提供独立 reference、Stata 19.5 验证 target 和日志；`ppmlhdfe` 安装后验证显式 FE 等价及分离剔除。新增 6 条 prompt，合计 14 个 skills、37 条 prompt；ADR-0006 记录 Heckman 的独立范围扩展。
...
### Fixed
- fix(验证): 10-commit code review findings — ① `judge.sh` 回显过滤只认 `. display` 前缀，`verify-dynamic-panel.do` 用单行 `if !.. display "SENTINEL"`（回显以 `. if` 开头）逃过过滤，导致装有 `xtabond2` 的机器被报成「未安装已跳过」；过滤放宽为「回显行且含 display」，do-file 统一改块形式，`test-harness.sh` 加两条形状锁。② 动态面板 marker 可被空诊断满足：`estat sargan` 在 `vce(robust)` 下只打印 "cannot calculate"、`_rc` 仍为 0，标记照样输出即判绿——改由 `assert e(sargan) < .` / `assert e(arm1) < . & e(arm2) < .` 托底，`estat abond` 标量名经 `ereturn list` 实测确认为 `e(arm1)`/`e(arm2)`（不是 `e(ar1)`）。③ judge 失败分支在 marker 缺失时吞掉 `r()` 计数，两类信息现合并输出。④ `test-prompts.sh` 的 `self_test_verify_log_resolution` 期望值由被测同一 primitive 现算（同义反复、永真），改为独立字面量 fixture 并补 `basics` 1:1 用例；删除 `run_prompts_mode` 中对纯函数 `plan_lines` 的重复赋值。⑤ `contract.sh` 的 `_contract_manifest_count` 与 `_contract_manifest_lines` 注释过滤规则漂移，删除已无调用方的后者并对齐规则。⑥ `community_check_dofile` 的 `o`/`c` 未声明 local，泄漏到 680 行的 `check-claims.sh` 全局。⑦ `dynamic-panel.md` 把 `estat sargan` 挂在 `vce(robust)` 之后（按其自身契约算不出结果），示例改为另跑非 robust 规格；`xtdpd` 段与新增 `cmclogit`/`cmmixlogit` 收敛断言、Heckman 两步与 ML 的样本量/方向交叉断言均经本机 Stata 19.5 重跑通过。
...
### Changed
- feat(验证) breaking(证据形态): verify raw log 不再入库 — 新增 ADR-0007 取代 ADR-0005。17 份 `verify/*.log` 取消跟踪（工作区副本保留，`run-verify.sh` 仍本地生成）并写入 `.gitignore`；证据改由 `.do` 内 `assert` 与 `VERIFY_MARKERS_REQUIRED` 标记承担，可在任意机器 `bash verify/run-verify.sh <skill>` 复现。动机是实测代价：一轮 10-commit review 的 ~5000 行新增里约 2400 行是 log，真实语义改动被淹没，且无任何机制校验 log 是否由当前 do-file 生成（stale 全绿 log 可长期掩盖回归）。`demo/logs/*.log` 仍入库（`check-claims.sh` 硬依赖）。同步 `AGENTS.md`（新增「诊断证据」惯例、ADR 计数 6→7）、`README.md`（首屏价值主张改为可重跑断言）、`CLAUDE.md`、`verify/run-verify.sh` 注释；`verify/test-acceptance.sh` 的 ADR-0005 锚点改判「取代关系已记录」，并加两条反回归锁：`.gitignore` 覆盖 `verify/*.log` + git 索引中不得出现该路径。
- feat(验证): `judge.sh` 的诊断标记契约改为声明驱动 — do-file 用 `display "VERIFY_MARKERS_REQUIRED=<M1> <M2> ..."` 自行声明所需标记，judge 只解释日志观察到的声明，不再硬编码 `DYNAMIC_PANEL_*` 名单（与 `community.sh` 对 package 名单的同一原则）。校验时先剔除声明行本身，避免 marker 被声明串自我满足。
