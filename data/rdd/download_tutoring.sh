#!/usr/bin/env bash
# ============================================================
# 下载 + 字节级校验 tutoring.dta（RDD 断点回归教学数据集）
#
# 来源  : https://github.com/quarcs-lab/data-open
#         (Carlos Mendez, *RDD in Stata: Evaluating a Tutoring Program* 官方 repo)
# 目标  : data/rdd/tutoring.dta
# 许可  : 教程配套开放数据（详见 data/rdd/README.md）
#
# 数据  : 1000 名学生；running variable = entrance_exam（0-100，cutoff=70）；
#         outcome = exit_exam；treatment = tutoring（score <= 70 自动参加）。
#         Sharp RDD 教学示例。
#
# 行为：
#   1. 通过 GitHub API 拿 base64 内容（避免 raw.githubusercontent.com 匿名 rate limit）
#   2. 解码写到目标路径
#   3. 字节数校验（必须等于 EXPECTED_SIZE，否则拒绝覆盖并报错）
#   4. `file` 命令确认是合法 Stata 数据文件
#
# 用法：
...
EXPECTED_SIZE=26029
REPO="quarcs-lab/data-open"
FILE="isds/tutoring.dta"
TARGET="$(cd "$(dirname "$0")" && pwd)/tutoring.dta"

echo "==> 下载 $REPO/$FILE 到 $TARGET ..."

# GitHub API：拿 base64 编码的内容（需 URL 全路径）
...
if size != expected:
    sys.exit(
        ...
        f"上游可能更新过文件。请先与团队确认是否更新 data/rdd/README.md 的"
