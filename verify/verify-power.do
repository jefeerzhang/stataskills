...
* data:     sim:200x10
* checks:   bloom-analytical+burlig-simulation+consistency
* ============================
...
display as result "  N = `N', T = `T', alpha = `alpha', power = `power_target'"
display as result "  Bloom MDE (in SD units) = " %6.4f `mde_sd'

* 断言：MDE 应在 [0.05, 0.15] SD 范围（与 Burlig-Preonas-Woerman 2020 Table 1 一致）
assert inrange(`mde_sd', 0.05, 0.15)
...
* （注：0.3 SD 是为了确保在 200 cluster × 10 period × 异质 ATT DGP 下 power > 0.5；
* 0.2 SD 太接近 MDE 0.0886 + ATT_g 异质性下 beta std 增大 → power 仅 ~0.33）
...
* simulation 用异质 ATT + 等相关结构 ATT=0.3 应能检测（power > 0.5）
* 一致性断言：Bloom 给的 MDE（均匀效应）应 < simulation 用的 ATT（异质 ATT）
* Bloom MDE ≈ 0.09 SD < 0.3 SD，断言成立
assert (`mde_sd' < 0.30)
